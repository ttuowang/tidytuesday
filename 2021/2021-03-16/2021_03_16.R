...
df %>%
  ...
  annotate("text", x = ymd("2012-07-15"), y = 2500, fontface="bold",
           label = "Average",family="Merriweather",color="#575757") +
  annotate("text", x = ymd("2012-07-15"), y = 4500, fontface="bold",
           label = "Peak",family="Merriweather",color="#82786f") +
  geom_line(data=filter(df, pandemic=='yes'),
            aes(x=date, y=avg_all_game),size=1.5, color="#fa5555")+
  geom_line(data=filter(df, pandemic=='yes'),
            aes(x=date, y=peak_all_game),size=1.5, color="#fa5555")+
  geom_point(data = filter(df, highlight==1),
             aes(x=date, y=avg_all_game),size=3, shape=21,fill="#fa5555",color="white")+
  geom_point(data = filter(df, highlight==1),
             aes(x=date, y=peak_all_game),size=3, shape=21,fill="#fa5555",color="white")+
  geom_label_repel(data = filter(df, highlight==1),hjust=1.3,vjust=-2,
  ...
  theme(
    ...
    plot.title = element_markdown(family = "luckiest", size = 20,
